...
void HAL_GPIO_EXTI_Callback(uint16_t gpio)
{
  ...
  if (current - last < osKernelGetTickFreq() / 5) // 200 ms
  {
    return;
  }
  ...
  if (gpio == GPIO_PIN_4)
  {
    message = MESSAGE_BUTTON_UP;
  }
  else if (gpio == GPIO_PIN_5)
  {
    message = MESSAGE_BUTTON_RIGHT;
  }
  else if (gpio == GPIO_PIN_6)
  {
    message = MESSAGE_BUTTON_DOWN;
  }
  else if (gpio == GPIO_PIN_7)
  {
    message = MESSAGE_BUTTON_LEFT;
  }
  else
  {
    return;
  ...
  osMessageQueuePut(channel, &message, 0, 0);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* add queues, ... */
  channel = osMessageQueueNew(8, sizeof(MessageType), NULL);
